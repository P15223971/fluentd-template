...
type logType struct {
	...
	FormatFirstLine string `yaml:"formatFirstLine,omitempty"`
...
var fns = template.FuncMap{
	"lastFieldInEntry": func(x int, a []fields) bool {
...
func GenTemplate(file string, config Config) {
	t, err := template.New("fluentd.yaml").Funcs(fns).ParseFiles("./templates/fluentd.yaml")
	panicError(err)
	s := strings.Split(file, ".")
	file = s[0]
	f, err := os.Create(fmt.Sprintf("../log-configs/fluentd-config/%v.conf", file))
	panicError(err)
...
func main() {
	...
	confList, err := ListConfigs("./template-configs/")
	if err != nil {
	...
	for _, config := range confList {
		var c Config
		c.ParseConfiguration("./template-configs/" + config)
		if err != nil {
